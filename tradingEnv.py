...
class TradingEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Dizionario per mappare le posizioni delle colonne (Risolve ChainedAssignmentError)
        col = {c: self.data.columns.get_loc(c) for c in self.data.columns}

        # CASE 1: LONG POSITION
        if(action == 1):
            self.data.iat[t, col['Position']] = 1
            # Case a: Long -> Long
            if(self.data['Position'].iloc[t - 1] == 1):
                self.data.iat[t, col['Cash']] = self.data['Cash'].iloc[t - 1]
                self.data.iat[t, col['Holdings']] = self.numberOfShares * self.data['Close'].iloc[t]
            # Case b: No position -> Long
                ...
                self.data.iat[t, col['Cash']] = self.data['Cash'].iloc[t - 1] - self.numberOfShares * self.data['Close'].iloc[t] * (1 + self.transactionCosts)
                self.data.iat[t, col['Holdings']] = self.numberOfShares * self.data['Close'].iloc[t]
                self.data.iat[t, col['Action']] = 1
            # Case c: Short -> Long
            else:
                self.data.iat[t, col['Cash']] = self.data['Cash'].iloc[t - 1] - self.numberOfShares * self.data['Close'].iloc[t] * (1 + self.transactionCosts)
                self.numberOfShares = math.floor(self.data['Cash'].iloc[t]/(self.data['Close'].iloc[t] * (1 + self.transactionCosts)))
                self.data.iat[t, col['Cash']] = self.data['Cash'].iloc[t] - self.numberOfShares * self.data['Close'].iloc[t] * (1 + self.transactionCosts)
                self.data.iat[t, col['Holdings']] = self.numberOfShares * self.data['Close'].iloc[t]
                self.data.iat[t, col['Action']] = 1
        ...
        # CASE 2: SHORT POSITION
            ...
            if(self.data['Position'].iloc[t - 1] == -1):
                ...
                if lowerBound <= 0:
                    ...
                    self.data.iat[t, col['Cash']] = self.data['Cash'].iloc[t - 1] - numberOfSharesToBuy * self.data['Close'].iloc[t] * (1 + self.transactionCosts)
                    self.data.iat[t, col['Holdings']] =  - self.numberOfShares * self.data['Close'].iloc[t]
                    customReward = True
        ...
        self.data.iat[t, col['Money']] = self.data['Holdings'].iloc[t] + self.data['Cash'].iloc[t]
        self.data.iat[t, col['Returns']] = (self.data['Money'].iloc[t] - self.data['Money'].iloc[t-1])/self.data['Money'].iloc[t-1]
        ...
        # Garantiamo l'output (Risolve l'unpacking NoneType)
